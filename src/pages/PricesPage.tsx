...
import { Tag, Users, Clock, Percent, Star, Calendar, Mountain, Baby } from "lucide-react";
...
const PricesPage = () => {
  ...
  type PriceRow = { period: string; price: string };
  type PriceCategory = { icon: LucideIcon; label: Record<Language, string>; prices: PriceRow[] };

  const priceCategories: PriceCategory[] = [
    {
      icon: Users,
      label: p.categories.groupCourse,
      prices: [
    ...
    {
      icon: Clock,
      label: p.categories.privateLessons,
      prices: [
    ...
    {
      icon: Star,
      label: p.categories.superCourses,
      prices: [
        { period: { it: "3 giorni (4h/giorno)", de: "3 Tage (4h/Tag)", en: "3 days (4h/day)" }[lang]!, price: "€ 250" },
        { period: { it: "5 giorni (4h/giorno)", de: "5 Tage (4h/Tag)", en: "5 days (4h/day)" }[lang]!, price: "€ 390" },
      ],
    },
    {
      icon: Baby,
      label: p.categories.fullDay,
      prices: [
        { period: { it: "1 giorno", de: "1 Tag", en: "1 day" }[lang]!, price: "€ 95" },
        { period: { it: "3 giorni", de: "3 Tage", en: "3 days" }[lang]!, price: "€ 250" },
        { period: { it: "5 giorni", de: "5 Tage", en: "5 days" }[lang]!, price: "€ 380" },
      ],
    },
    {
      icon: Calendar,
      label: p.categories.weekendCourse,
      prices: [
        { period: { it: "3 giorni (Ven-Dom)", de: "3 Tage (Fr-So)", en: "3 days (Fri-Sun)" }[lang]!, price: "€ 180" },
      ],
    },
    {
      icon: Mountain,
      label: p.categories.freeride,
      prices: [
        { period: { it: "Giornata (1 pers.)", de: "Ganztag (1 Pers.)", en: "Full day (1 pers.)" }[lang]!, price: "€ 350" },
        { period: { it: "Giornata (2 pers.)", de: "Ganztag (2 Pers.)", en: "Full day (2 pers.)" }[lang]!, price: "€ 220/pers." },
      ],
    },
  ];
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="p-5 border-b border-border" style={{ background: "var(--gradient-alpine)" }}>
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-display text-base font-bold text-primary-foreground">{t(cat.label, lang)}</h3>
                  </div>
                </div>
                <div className="divide-y divide-border">
                  {cat.prices.map((price, j) => (
                    <div key={j} className="flex justify-between items-center px-6 py-3">
                      <span className="text-foreground font-body text-sm">{price.period}</span>
                      <span className="font-display font-bold text-primary">{price.price}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
